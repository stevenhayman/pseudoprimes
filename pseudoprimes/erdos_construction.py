...
def construct_carmichael_number(
    ...
    """Function which constructs a Carmichael number using the Erdos construction."""
    primes = build_primes_set(factorization)
    return find_subset(primes)


def build_primes_set(factorization: Factorization) -> Set[int]:
    """Function which builds the set of prime numbers used by the Erdos construction."""
    primes: Set[int] = set()
